G5ClientHub.NavWidgetsDropTargetView = Ember.View.extend G5ClientHub.Droppable,
  ...
  dropTargetAction: Ember.computed((key, value) ->
    ...
    if Ember.isEmpty(@get("dragContext"))
      ...
      return null
    unless @get("dragContext.isAddedToNavWidgets")
      @set "helpText", "(Drop to Add)"
      "drop-target-add"
    else if @get("dragContext.isAddedToNavWidgets")
  ...
  drop: (event) ->
    ...
    Ember.run.next this, ->
      view.set "content.isAddedToNavWidgets", not view.get("content.isAddedToNavWidgets")
