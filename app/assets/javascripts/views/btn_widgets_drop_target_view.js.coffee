G5ClientHub.BtnWidgetsDropTargetView = Ember.View.extend G5ClientHub.Droppable,
  tagName: "div" # create a div
  classNames: ["btnWidgetsDropTarget"] # that always has this class
  ...
  dropTargetAction: Ember.computed((key, value) ->
    ...
    if Ember.isEmpty(@get("dragContext"))
      ...
      return null
    unless @get("dragContext.isAddedToBtnWidgets")
      @set "helpText", "(Drop to Add)"
      "drop-target-add"
    else if @get("dragContext.isAddedToBtnWidgets")
  ...
  drop: (event) ->
    ...
    Ember.run.next this, ->
      view.set "content.isAddedToBtnWidgets", not view.get("content.isAddedToBtnWidgets")
