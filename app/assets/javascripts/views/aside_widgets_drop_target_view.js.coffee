G5ClientHub.AsideWidgetsDropTargetView = Ember.View.extend G5ClientHub.Droppable,
  tagName: "div" # create a div
  classNames: ["asideWidgetsDropTarget"] # that always has this class
  ...
  dropTargetAction: Ember.computed((key, value) ->
    if Ember.isEmpty(@get("dragContext"))
      ...
      return null
    unless @get("dragContext.isAddedToAsideWidgets")
      @set "helpText", "(Drop to Add)"
      "drop-target-add"
    else if @get("dragContext.isAddedToAsideWidgets")
  ...
  drop: (event) ->
    ...
    Ember.run.next this, ->
      view.set "content.isAddedToAsideWidgets", not view.get("content.isAddedToAsideWidgets")
